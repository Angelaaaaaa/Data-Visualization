...
```{r}
library(MASS)
# Tile: Support Functions and Datasets for Venables and Ripley's MASS
```
...
head(lab1data)
#head(lab1data) finds the first part of data in lab1data
```

b.  How many rows does ```lab1data``` have?  Hint:  ```help(nrow)```
```{r}
nrow(lab1data)
#there are 27 rows in lab1data
```

c.  What happens when you add ```, nrows = 10``` inside the parentheses in the line of code in (a)?  (Hint:  See ```help(read.table)```.)
```{r}
lab1data <- read.table("https://raw.githubusercontent.com/sventura/315-code-and-datasets/master/data/lab1.txt", header = T, nrow = 10)
nrow(lab1data)
#After we add nrow = 10, then it only read in 10 rows of the data
```
...
```{r}
v1 <- c(1,2,3,4)
myvector <- c(1,4,9, 16,25,36,49,64,81,100,121,169,196)
```
...
row4 <- lab1data[4,]
min(row4)
#the min of row 4 is 1
max(row4)
#the max of row 4 is 252
```
...
column4 <- lab1data[,4]
min(column4)
#the min of column 4 is 1
max(column4)
#the max of column 4 is 244
```
